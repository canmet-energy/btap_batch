...
class BTAPSamplingLHS(BTAPParametric):
    def compute_scenarios(self):
        ...
        np.random.seed(self.algorithm_lhs_random_seed)

        # Create the lhs algorithm.
        lhs = Lhs(lhs_type=self.algorithm_lhs_type, criterion=None)
        # Get samples
        samples = lhs.generate(space.dimensions, n_samples=self.algorithm_lhs_n_samples)
